...
class Trader:
    ...
    def compute_orders_regression(order_depths, position, product, acc_bid, acc_ask, trader_data: TraderData):
        ...
        for ask, vol in sell_orders.items():
            ...
            if ask <= acc_bid or (ask == acc_bid + 1 and position <= -5):
                ...
                order_price = ask
                curr_pos += order_volume
                orders.append(Order(product, round(order_price), order_volume))
    ...
    def compute_orders_orchids(order_depths, position, observations, own_trades, trader_data: TraderData):
        ...
        if curr_pos > -pos_limit:
            ...
            order_price = round(conv_buy_pr) + 1
            curr_pos += order_volume

            if timestamp > 0:
                prev_sunlight = trader_data.decode_json("prev_sunlight")
                prev_humidity = trader_data.decode_json("prev_humidity")
                # humidity_signal = (((observations.humidity - prev_humidity > 0) and (prev_humidity > 80))
                #                    or ((observations.humidity - prev_humidity < 0) and (prev_humidity < 60)))
                # sunlight_signal = (((observations.sunlight - prev_sunlight) < 0) and (prev_sunlight < 2500))
                # if humidity_signal and sunlight_signal:
                #     order_price = round(conv_buy_pr + 1.8)
                # elif humidity_signal:
                #     order_price = round(conv_buy_pr + 1.6)
            orders.append(Order(product, order_price, order_volume))
        ...
        trader_data.update_values("prev_sunlight", observations.sunlight)
        trader_data.update_values("prev_humidity", observations.humidity)
        # print(conversions)
    ...
    def compute_orders_basket(order_depths, positions, trader_data: TraderData):
        ...
        print("Basket positions: " + str(basket_pos))
        ...
        if timestamp > 200:
            ...
            if curr_spread > basket_mean + 1.5 * basket_std:
                ...
                if vol > 0:
                    orders.append(Order('GIFT_BASKET', best_buy['GIFT_BASKET'], -vol))
            elif curr_spread < basket_mean - 1.5 * basket_std:
                vol = pos_limits['GIFT_BASKET'] - basket_pos
                if vol > 0:
                    orders.append(Order('GIFT_BASKET', best_sell['GIFT_BASKET'], vol))
    ...
    def compute_orders_chocolate(order_depths, positions, trader_data: TraderData):
        ...
        choco_price = (mid_price['GIFT_BASKET'] -
                       (6 * mid_price['STRAWBERRIES'] + 4 * mid_price['CHOCOLATE'] + mid_price['ROSES']))
        ...
        if timestamp > 200:
            ...
            print("Mean: ", choco_mean)
            print("Std: ", choco_std)

            if (curr_spread > basket_mean + 1.5 * basket_std
                    and choco_price > choco_mean + 1.5 * choco_std):
                vol = choco_pos + pos_limits['CHOCOLATE']
                if vol > 0:
                    orders.append(Order('CHOCOLATE', best_buy['CHOCOLATE'], -vol))

            elif (curr_spread < basket_mean - 1.5 * basket_std
                  and choco_price < choco_mean - 1.5 * choco_std):
                vol = pos_limits['CHOCOLATE'] - choco_pos
                if vol > 0:
                    orders.append(Order('CHOCOLATE', best_sell['CHOCOLATE'], vol))
    ...
    def compute_orders_roses(order_depths, positions, trader_data: TraderData):
        ...
        roses_price = (mid_price['GIFT_BASKET'] -
                       (6 * mid_price['STRAWBERRIES'] + 4 * mid_price['CHOCOLATE'] + mid_price['ROSES']))

        roses_sum = trader_data.decode_json("real_basket_sum")
        roses_sq_sum = trader_data.decode_json("real_basket_sq_sum")
        ...
        if timestamp > 0:
            ...
            print("Roses Basket std: " + str(basket_std))
            print("Roses std: " + str(roses_std))

            if (curr_spread > basket_mean + 1.5 * basket_std
                    and roses_price > roses_mean + 1.5 * roses_std):
                vol = roses_pos + pos_limits['ROSES']
                if vol > 0:
                    v1 = round(-vol / 3)
                    orders.append(Order('ROSES', min(best_buy['ROSES'], worst_buy['ROSES'] + 1), v1))
                    orders.append(Order('ROSES', min(best_buy['ROSES'], worst_buy['ROSES']), -vol - v1))
                ...
                if vol > 0:
                    v1 = round(vol / 3)
                    orders.append(Order('ROSES', max(best_buy['ROSES'], worst_sell['ROSES'] - 1), v1))
                    orders.append(Order('ROSES', max(best_buy['ROSES'], worst_sell['ROSES']), vol - v1))
        ...
        trader_data.update_values("real_basket_sum", roses_sum + roses_price)
        trader_data.update_values("real_basket_sq_sum", roses_sq_sum + roses_price ** 2)

        return orders

    @staticmethod
    def compute_orders_strawberries_bots(order_depths, position, market_trades, trader_data: TraderData):
        pos_limit = 350
        timestamp = round(trader_data.decode_json("timestamp") / 100)
        orders = []
        for trade in market_trades:
            if trade.symbol == 'STRAWBERRIES' and trade.seller == 'Remy' and round(
                    trade.timestamp / 100) >= timestamp - 1:
                trader_data.update_values("to_buy_strawberries", False)
                trader_data.update_values("to_trade_strawberries", trade.quantity)
            if trade.symbol == 'STRAWBERRIES' and trade.buyer == 'Remy' and round(
                    trade.timestamp / 100) >= timestamp - 1:
                trader_data.update_values("to_buy_strawberries", True)
                trader_data.update_values("to_trade_strawberries", trade.quantity)
        to_buy_strawberries = trader_data.decode_json("to_buy_strawberries")
        to_trade_strawberries = trader_data.decode_json("to_trade_strawberries")
        if to_buy_strawberries is None:
            return orders
        ...
        mid_price = round((best_ask + best_bid) / 2)

        if to_buy_strawberries is True:
            to_trade_volume = min(pos_limit - position, to_trade_strawberries)
            orders.append(Order("STRAWBERRIES", mid_price - 1, to_trade_volume))
        elif to_buy_strawberries is False:
            to_trade_volume = min(position - pos_limit, to_trade_strawberries)
            orders.append(Order("STRAWBERRIES", mid_price + 1, -to_trade_volume))
        return orders
    ...
    def init(trader_data: TraderData):
        ...
        trader_data.add_object_encoding("to_buy_strawberries", None)
        trader_data.add_object_encoding("to_trade_strawberries", 0)
    ...
    def run(self, state: TradingState):
        ...
        final_orders = {"AMETHYSTS": [], "STARFRUIT": [], "ORCHIDS": [], "GIFT_BASKET": [], "CHOCOLATE": [],
                        "STRAWBERRIES": [], "ROSES": [], "COCONUT_COUPON": []}
        ...
        final_orders["STRAWBERRIES"] += (self.compute_orders_strawberries_bots(
            state.order_depths["STRAWBERRIES"],
            state.position["STRAWBERRIES"] if "STRAWBERRIES" in state.position else 0,
            state.market_trades["STRAWBERRIES"] if "STRAWBERRIES" in state.market_trades else [],
            trader_data
        ))
